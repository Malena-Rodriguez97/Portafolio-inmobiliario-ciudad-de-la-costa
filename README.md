# Portafolio inmobiliario - Ciudad de la Costa

Análisis académico de 555 propiedades en venta y alquiler en 6 barrios de Ciudad 
de la Costa. 

Incluye una simulación de consultoría empresarial y el desarrollo técnico completo del proceso.

Fuente: Mercado Libre Uruguay. 

Herramientas: BigQuery + SQL + Google Colab + Looker Studio.

Este proyecto se divide en dos secciones según el perfil de quien lo 
consulta: negocio/inversión y proceso técnico/metodológico.


## Sección Empresarial
Para perfiles de negocio, inversión y BI.

- Resumen ejecutivo: síntesis de una página.

  https://docs.google.com/document/d/1PSLmwgG4uZRZ_7db5qNMPYIZUpfEPzRmPPJuceNAGX4/edit?usp=sharing

  
- Presentación: hallazgos principales.

  https://drive.google.com/file/d/198-Tsq_bE5TNpcVRqqPWywXopEtegBz7/view?usp=sharing

  
- Dashboard interactivo: panel con métricas relevantes segmentadas por hojas.

  https://datastudio.google.com/reporting/13a643a9-5dfc-4ad5-bafe-eb19910652b9

  
- Guía de uso del panel: cómo navegar el dashboard.

  https://docs.google.com/document/d/1fnCKaBAu1kHJjNGDWdM6PZTyPLwsk0CYeY_r5S7P7Ls/edit?usp=sharing

  
- Informe de consolidación de hallazgos: conclusiones y recomendaciones para perfil de inversor.

https://docs.google.com/document/d/1aD7R886ytZHlxEzsMcSlq0C39gnjEzMIkawCLagRcxo/edit?usp=sharing


## Sección Metodológica
Para quienes quieren ver el proceso técnico y metodológico completo.

- Navegación académica: índice de qué contiene cada notebook.

  https://docs.google.com/document/d/1f_0UlJfloWyq_laefkO3sraI1qrEi6fSDnyjk1iOLM4/edit?usp=sharing

- Notebooks (5 módulos): proceso de análisis paso a paso, de la limpieza de datos a la historización.

  https://github.com/Malena-Rodriguez97/Portafolio-inmobiliario-ciudad-de-la-costa/tree/main/notebooks

- Manual educativo de SQL: guía de las consultas usadas en el proyecto.

  https://drive.google.com/file/d/11GNC1zG72PgXN402PV0DwvuUzCIeYhT0/view?usp=sharing
    
- Ficha técnica del panel: fuente de datos, dimensiones y métricas de cada visual.

  https://docs.google.com/document/d/11GQ0xTzCBDPsQiJSh-49vigQQsUFSqRkG_nRKuXzHss/edit?usp=sharing

  
Malena Rodríguez — Análisis de Datos & BI
 Ciudad de la Costa, Uruguay — Julio 2026
